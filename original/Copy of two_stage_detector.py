...
class DetectorBackboneWithFPN(nn.Module):
    ...
    def __init__(self, out_channels: int):
        ...
        for level_name, feature_shape in dummy_out_shapes:
            self.lat_layer[f"{level_name}"] = nn.Conv2d(in_channels=feature_shape[1], out_channels=self.out_channels, kernel_size=1, stride=1, padding=0)
            self.fpn_layer["p"+f"{level_name}"] = nn.Conv2d(in_channels=out_channels, out_channels=self.out_channels, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, images: torch.Tensor):
        ...
        for key, value in backbone_feats.items():
          backbone_feats_out[key] = value

        c3 = backbone_feats_out["c3"]
        c4 = backbone_feats_out["c4"]
        c5 = backbone_feats_out["c5"]
        
        device = images.device
        
        
        self.lat_layer["c5"].to(device)
        c5_lat = self.lat_layer["c5"](c5)
        p5 = c5_lat
        self.fpn_layer["pc5"].to(device)
        fpn_feats["p5"] = self.fpn_layer["pc5"](p5)
                
        self.lat_layer["c4"].to(device)
        c4_lat = self.lat_layer["c4"](c4)
        _,_,H,W= c4_lat.shape
        p4 = F.interpolate(p5, size=(H,W), mode='bilinear') + c4_lat
        self.fpn_layer["pc4"].to(device)
        fpn_feats["p4"] = self.fpn_layer["pc4"](p4)
        
        self.lat_layer["c3"].to(device)
        c3_lat = self.lat_layer["c3"](c3)
        _,_,H,W= c3_lat.shape
        p3 = F.interpolate(p4, size=(H,W), mode='bilinear') + c3_lat
        self.fpn_layer["pc3"].to(device)
        fpn_feats["p3"] = self.fpn_layer["pc3"](p3)   
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
...
class RPNPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for i in range(num_layers):
            C_out = stem_channels[i]
            conv_layer = nn.Conv2d(in_channels=C_in, out_channels=C_out, kernel_size=3, stride=1, padding=1)
            relu_layer = nn.ReLU()
            conv_layer.weight.data = 0.01*torch.randn(C_out,C_in,3,3)
            conv_layer.bias.data = torch.zeros(C_out)
            C_in = C_out
        ...
        # Replace "pass" statement with your code
        self.pred_obj = nn.Conv2d(in_channels=C_out, out_channels=num_anchors, kernel_size=1, stride=1, padding=0)
        self.pred_box = nn.Conv2d(in_channels=C_out, out_channels=4*num_anchors, kernel_size=1, stride=1, padding=0)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for key, value in feats_per_fpn_level.items():
          stem_out = self.stem_rpn(value)
          
          pred_obj = self.pred_obj(stem_out)
          pred_box = self.pred_box(stem_out)
          
          N, C, H, W = pred_obj.shape
          pred_obj_reshaped = pred_obj.permute(0, 2, 3, 1).reshape(N,H*W,C)
          
          object_logits[key] = pred_obj_reshaped
          
          N, C, H, W = pred_box.shape
          pred_box_reshaped = pred_box.permute(0, 2, 3, 1).reshape(N,H*W,C)
          
          
          boxreg_deltas[key] = pred_box_reshaped
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
def get_fpn_location_coords(
    ...
    for level_name, feat_shape in shape_per_fpn_level.items():
        ...
        _,_,H, W = feat_shape        
        loc_tensor = torch.zeros(H*W,2)
        
        id = 0
        for i in range(W):
            for j in range(H):
                loc_tensor[id,:] = torch.tensor([level_stride * (i + 0.5), level_stride * (j + 0.5)])
                id +=1
        location_coords[level_name] = loc_tensor
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
...
def generate_fpn_anchors(
    ...
    for level_name, locations in locations_per_fpn_level.items():
        ...
        for aspect_ratio in aspect_ratios:
            ...
            width = torch.sqrt(torch.tensor(anchor_area/aspect_ratio))
            
            height = (anchor_area/width).clone().detach()
            loc_coord = locations_per_fpn_level[level_name]
            width_height_tensor = torch.tensor([width,height])
            top_left_coord = loc_coord - 0.5*width_height_tensor
            bottom_right_coord = loc_coord + 0.5*width_height_tensor
            
            anchor_box = torch.cat((top_left_coord,bottom_right_coord),dim=1)
...
def iou(boxes1: torch.Tensor, boxes2: torch.Tensor) -> torch.Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    iou = torchvision.ops.box_iou(boxes1.to(boxes2.device),boxes2)
...
def rcnn_get_deltas_from_anchors(
    ...
    deltas = None
    # Replace "pass" statement with your code
    gt_boxes_ctr_x = (gt_boxes[:,0]+gt_boxes[:,2])/2
    gt_boxes_ctr_y = (gt_boxes[:,1]+gt_boxes[:,3])/2
    
    anchors_ctr_x = (anchors[:,0]+anchors[:,2])/2
    anchors_ctr_y = (anchors[:,1]+anchors[:,3])/2
    
    delta_pos_x = gt_boxes_ctr_x - anchors_ctr_x
    delta_pos_y = gt_boxes_ctr_y - anchors_ctr_y
    
    gt_boxes_width = torch.abs(gt_boxes[:,0]-gt_boxes[:,2])
    gt_boxes_height = torch.abs(gt_boxes[:,1]-gt_boxes[:,3])
    
    anchors_width = torch.abs(anchors[:,0]-anchors[:,2])
    anchors_height = torch.abs(anchors[:,1]-anchors[:,3])
    
    delta_width_scale = gt_boxes_width/anchors_width
    delta_height_scale = gt_boxes_height/anchors_height
    
    delta_pos_x = delta_pos_x.reshape(-1,1)
    delta_pos_y = delta_pos_y.reshape(-1,1)
    delta_width_scale = delta_width_scale.reshape(-1,1)
    delta_height_scale = delta_height_scale.reshape(-1,1)
    deltas = torch.cat((delta_pos_x,delta_pos_y,delta_width_scale,delta_height_scale), dim=1)
    
    dummy = -1*torch.ones_like(gt_boxes[:,:4])
    deltas[gt_boxes[:,:4] <= dummy] = -1e8
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def rcnn_apply_deltas_to_anchors(
    ...
    with torch.autograd.set_detect_anomaly(True):
        ...
        anchors_scaled_width = deltas[:,2]*anchors_width
        anchors_scaled_height = deltas[:,3]*anchors_height
        
        anchors_ctr_x = (anchors[:,0]+anchors[:,2])/2
        anchors_ctr_y = (anchors[:,1]+anchors[:,3])/2
        
        anchors_shift_ctr_x = anchors_ctr_x + deltas[:,0]
        anchors_shift_ctr_y = anchors_ctr_y + deltas[:,1]
        
       
        anchors_shift_top_left_x = anchors_shift_ctr_x - 0.5*anchors_scaled_width
        anchors_shift_top_left_y = anchors_shift_ctr_y - 0.5*anchors_scaled_height
        
        anchors_shift_bot_right_x = anchors_shift_ctr_x + 0.5*anchors_scaled_width
        anchors_shift_bot_right_y = anchors_shift_ctr_y + 0.5*anchors_scaled_height
        
        anchors_shift_top_left_x = anchors_shift_top_left_x.reshape(-1,1)
        anchors_shift_top_left_y = anchors_shift_top_left_y.reshape(-1,1)
        anchors_shift_bot_right_x = anchors_shift_bot_right_x.reshape(-1,1)
        anchors_shift_bot_right_y = anchors_shift_bot_right_y.reshape(-1,1)
    
        anchors_shift_top_left_x = torch.where(anchors_shift_top_left_x >= 1e8, -anchors_shift_top_left_x, anchors_shift_top_left_x)
        anchors_shift_top_left_y = torch.where(anchors_shift_top_left_y >= 1e8, -anchors_shift_top_left_y, anchors_shift_top_left_y)
        anchors_shift_bot_right_x = torch.where(anchors_shift_bot_right_x >= 1e8, -anchors_shift_bot_right_x, anchors_shift_bot_right_x)
        anchors_shift_bot_right_y = torch.where(anchors_shift_bot_right_y >= 1e8, -anchors_shift_bot_right_y, anchors_shift_bot_right_y)
    
        output_boxes = torch.cat((anchors_shift_top_left_x,anchors_shift_top_left_y,anchors_shift_bot_right_x,anchors_shift_bot_right_y),dim=1)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
    return output_boxes
...
def nms(boxes: torch.Tensor, scores: torch.Tensor, iou_threshold: float = 0.5):
    ...
    prod_area = torch.prod(boxes[:, 2:] - boxes[:, :2], dim=1)    
    while sorted_indexes.size()[0] > 0:
        idx = sorted_indexes[0]
        box_high_score = boxes[idx]     
        keep.append(idx)
    
        top_left = torch.max(box_high_score[:2], boxes[sorted_indexes][:, :2])    
        bot_right = torch.min(box_high_score[2:], boxes[sorted_indexes][:, 2:])    
        intersect = torch.prod(bot_right-top_left, dim=1)*(top_left<bot_right).all(dim=1)
    
        p = prod_area[idx] 
        q = prod_area 
        IoU = torch.div(intersect, p+q[sorted_indexes]-intersect).squeeze()

        mask = torch.where(IoU <= iou_threshold)
...
class RPN(nn.Module):
    ...
    def forward(
        ...
        shapes_per_fpn_level["p4"] = feats_per_fpn_level["p4"].shape
        shapes_per_fpn_level["p5"] = feats_per_fpn_level["p5"].shape
        ...
        anchors_per_fpn_level = generate_fpn_anchors(location_coords, strides_per_fpn_level, self.anchor_stride_scale)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        for i in range(B):
          mtch_gt_box = rcnn_match_anchors_to_gt(anchor_boxes,gt_boxes[i,:,:],self.anchor_iou_thresholds)
          matched_gt_boxes.append(mtch_gt_box)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        if self.training:
            ...
            # Replace "pass" statement with your code
            fg_idx, bg_idx = sample_rpn_training(matched_gt_boxes, num_samples=self.post_nms_topk // 2, fg_fraction=0.5)
            
            tr_boxreg_deltas = torch.cat((pred_boxreg_deltas[fg_idx],pred_boxreg_deltas[bg_idx]),dim=0)
            
            gt_deltas = torch.zeros_like(tr_boxreg_deltas)
            
            anchor_boxes = anchor_boxes.to(matched_gt_boxes.device)
            gt_targets = rcnn_get_deltas_from_anchors(anchor_boxes, matched_gt_boxes)
            all_idxs = torch.cat((fg_idx, bg_idx))
            box_targets = gt_targets[all_idxs]
            
            loss_box = F.l1_loss(tr_boxreg_deltas, box_targets, reduction="none")
            loss_box[box_targets == -1e8] *= 0.0
            
            
            
            tr_pred_obj_logits = torch.cat((pred_obj_logits[fg_idx],pred_obj_logits[bg_idx]),dim=0)
            
            gt_objectness = torch.cat((torch.ones_like(pred_obj_logits[fg_idx]), torch.zeros_like(pred_obj_logits[bg_idx])))
            
            loss_obj = F.binary_cross_entropy_with_logits(tr_pred_obj_logits.view(-1), gt_objectness, reduction="none")
    ...
    def predict_proposals(
        ...
        for _batch_idx in range(batch_size):
            ...
            for level_name in anchors_per_fpn_level.keys():
                ...
                level_boxreg_deltas = level_boxreg_deltas.reshape(-1,4)
                level_anchors = level_anchors.to(level_boxreg_deltas.device)
                
                moved_anchors = rcnn_apply_deltas_to_anchors(level_boxreg_deltas, level_anchors)
                moved_anchors_ctr_x_coord = (moved_anchors[:,0]+moved_anchors[:,2])/2
                moved_anchors_ctr_y_coord = (moved_anchors[:,1]+moved_anchors[:,3])/2
                moved_anchors_tl_x_coord = (moved_anchors_ctr_x_coord - (image_size[0]/2)).reshape(-1,1)
                moved_anchors_tl_y_coord = (moved_anchors_ctr_y_coord - (image_size[1]/2)).reshape(-1,1)
                
                moved_anchors_br_x_coord = (moved_anchors_ctr_x_coord + (image_size[0]/2)).reshape(-1,1)
                moved_anchors_br_y_coord = (moved_anchors_ctr_y_coord + (image_size[1]/2)).reshape(-1,1)
                
                all_moved_anchors = torch.cat((moved_anchors_tl_x_coord,moved_anchors_tl_y_coord,moved_anchors_br_x_coord,moved_anchors_br_y_coord),dim=1)
                level_obj_logits = level_obj_logits.reshape(-1)
                lvl_obj_lgt = level_obj_logits.reshape(-1,1)
                len_obj_lgt = len(lvl_obj_lgt)
                if self.pre_nms_topk<len_obj_lgt:
                  pre_nms = self.pre_nms_topk
                else:
                  pre_nms = len_obj_lgt
                _, top_k_scores_idx = lvl_obj_lgt.topk(pre_nms,dim=0)
                
                proposals_per_fpn_level_per_image[level_name] = torch.cat((moved_anchors_tl_x_coord,moved_anchors_tl_y_coord,moved_anchors_br_x_coord,moved_anchors_br_y_coord),dim=1)
                
                scores_per_fpn_level_per_image[level_name] = level_obj_logits
                ##############################################################
                #                        END OF YOUR CODE                    #
                ##############################################################
...
class FasterRCNN(nn.Module):
    ...
    def __init__(
        ...
        cls_pred.append(nn.Linear(roi_size[0]*roi_size[1]*self.stem_channels[-1], self.num_classes+1))
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
    ...
    def forward(
        ...
        for _idx in range(len(gt_boxes)):
            ...
            # Replace "pass" statement with your code
            rcnn_matched_anchors = rcnn_match_anchors_to_gt(proposals_per_image, gt_boxes_per_image,(0.5,0.5))
            matched_gt_boxes.append(rcnn_matched_anchors)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        labels = matched_gt_boxes[torch.cat((fg_idx,bg_idx)),-1] + 1
        
        pred = pred_cls_logits[torch.cat((fg_idx,bg_idx))]
        
        labels = labels.to(torch.uint8)
        
        loss_cls = F.cross_entropy(pred, labels)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        return {
    ...
    def inference(
        ...
        # Replace "pass" statement with your code
        cls_scrs = F.softmax(pred_cls_logits, dim=1)
        
        pred_scores, pred_classes = cls_scrs.topk(1, dim=1)

        idx = (pred_scores > test_score_thresh).squeeze()
        pred_scores = pred_scores[idx].squeeze()
        
        pred_boxes = pred_boxes[idx]
        pred_classes = pred_classes[idx].squeeze()-1
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
