...
def train(args):
    ...
    props = PROPSDataset('train',
                     im_dir=dataset_config['im_train_path'],
                     ann_dir=dataset_config['ann_train_path'])

    train_dataset = DataLoader(props,
                               batch_size=4,
                               shuffle=True,
                               num_workers=4,
                               collate_fn=collate_function)
    ...
    # Initialize dictionary to save losses
    epoch_losses = {"rpn_classification": [], "rpn_localization": [], 
                    "frcnn_classification": [], "frcnn_localization": []}  # Added
    ...
    for i in range(num_epochs):
        ...
        # Save mean loss values for the epoch
        epoch_losses["rpn_classification"].append(np.mean(rpn_classification_losses))  # Added
        epoch_losses["rpn_localization"].append(np.mean(rpn_localization_losses))      # Added
        epoch_losses["frcnn_classification"].append(np.mean(frcnn_classification_losses))  # Added
        epoch_losses["frcnn_localization"].append(np.mean(frcnn_localization_losses))      # Added

    # Save losses to a file
    with open(os.path.join(train_config['task_name'], "epoch_losses.npy"), "wb") as f:  # Added
        np.save(f, epoch_losses)  # Added
    print('Done Training...')
