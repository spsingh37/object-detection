...
from dataset.props import PROPSDataset
...
def load_model_and_dataset(args):
    ...
    props = PROPSDataset('test', im_dir=dataset_config['im_test_path'], ann_dir=dataset_config['ann_test_path'])
    test_dataset = DataLoader(props, batch_size=1, shuffle=False)
    ...
    return faster_rcnn_model, props, test_dataset
...
def infer(args):
    ...
    faster_rcnn_model, props, test_dataset = load_model_and_dataset(args)

    for sample_count in tqdm(range(50)):
        # random_idx = random.randint(0, len(voc))
        random_idx = random.randint(0, len(props))
        # im, target, fname = voc[random_idx]
        im, target, fname = props[random_idx]
        ...
        for idx, box in enumerate(target['bboxes']):
            ...
            text = props.idx2label[target['labels'][idx].detach().cpu().item()]
            ...
            cv2.putText(gt_im, text=props.idx2label[target['labels'][idx].detach().cpu().item()],
                        org=(x1 + 5, y1 + 15),
                        thickness=1,
                        fontScale=1,
                        color=[0, 0, 0],
        ...
        for idx, box in enumerate(boxes):
            ...
            text = '{} : {:.2f}'.format(props.idx2label[labels[idx].detach().cpu().item()],
                                        scores[idx].detach().cpu().item())
            text_size, _ = cv2.getTextSize(text, cv2.FONT_HERSHEY_PLAIN, 1, 1)
            text_w, text_h = text_size
...
def evaluate_map(args):
    ...
    for im, target, fname in tqdm(test_dataset):
        ...
        for label_name in props.label2idx:
            pred_boxes[label_name] = []
            gt_boxes[label_name] = []
        ...
        for idx, box in enumerate(boxes):
            ...
            label = labels[idx].detach().cpu().item()
            score = scores[idx].detach().cpu().item()
            label_name = props.idx2label[label]
            pred_boxes[label_name].append([x1, y1, x2, y2, score])
        for idx, box in enumerate(target_boxes):
            x1, y1, x2, y2 = box.detach().cpu().numpy()
            label = target_labels[idx].detach().cpu().item()
    ...
    for idx in range(len(props.idx2label)):
        print('AP for class {} = {:.4f}'.format(props.idx2label[idx], all_aps[props.idx2label[idx]]))
