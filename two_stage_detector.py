...
class RPNPredictionNetwork(nn.Module):
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for key, value in feats_per_fpn_level.items():
          ...
          pred_box_reshaped = pred_box.permute(0, 2, 3, 1).reshape(N,H*W*3,4)
...
def rcnn_get_deltas_from_anchors(
    ...
    gt_boxes_width = gt_boxes[:,2]-gt_boxes[:,0]
    gt_boxes_height = gt_boxes[:,3]-gt_boxes[:,1]
    
    anchors_width = anchors[:,2]-anchors[:,0]
    anchors_height = anchors[:,3]-anchors[:,1]
    
    delta_width_scale = torch.log(gt_boxes_width/anchors_width)
    delta_height_scale = torch.log(gt_boxes_height/anchors_height)
    
    delta_pos_x = (delta_pos_x/anchors_width).reshape(-1,1)
    delta_pos_y = (delta_pos_y/anchors_height).reshape(-1,1)
...
def rcnn_apply_deltas_to_anchors(
    ...
    anchors_shift_ctr_x = anchors_ctr_x + deltas[:,0]*anchors_width
    anchors_shift_ctr_y = anchors_ctr_y + deltas[:,1]*anchors_height
    
    anchors_scaled_width = torch.exp(deltas[:,2])*anchors_width
    anchors_scaled_height = torch.exp(deltas[:,3])*anchors_height
...
def nms(boxes: torch.Tensor, scores: torch.Tensor, iou_threshold: float = 0.5):
    ...
    while sorted_indexes.size()[0] > 0:
        ...
        IoU = iou(box_high_score.reshape(1,-1), boxes[sorted_indexes])
        IoU = IoU.reshape(-1)
...
class RPN(nn.Module):
    ...
    def forward(
        ...
        if self.training:
            ...
            gt_trg = rcnn_get_deltas_from_anchors(anchor_boxes, matched_gt_boxes)
            fg_bg_idxs = torch.cat((fg_idx, bg_idx))
            target_boxes = gt_trg[fg_bg_idxs]
            
            loss_box = F.l1_loss(tr_boxreg_deltas, target_boxes, reduction="none")
            loss_box[target_boxes == -1e8] *= 0.0
    ...
    def predict_proposals(
        ...
        for _batch_idx in range(batch_size):
            ...
            for level_name in anchors_per_fpn_level.keys():
                ...
                min_limit = torch.tensor([0.,0.,0.,0.], device=level_boxreg_deltas.device)
                max_limit = torch.tensor([image_size[0], image_size[1], image_size[0], image_size[1]],device=level_boxreg_deltas.device)
                moved_anchors_cropped = torch.clamp(moved_anchors, min=min_limit,max=max_limit)
                
                level_obj_logits = level_obj_logits.reshape(-1)
                
                if len(level_obj_logits) > self.pre_nms_topk:
                    level_obj_logits, idxs_top = torch.topk(level_obj_logits, self.pre_nms_topk)
                    moved_anchors_cropped = moved_anchors_cropped[idxs_top]
                else:
                    pass
                idxs_post_nms = torchvision.ops.nms(moved_anchors_cropped, level_obj_logits,iou_threshold= self.nms_thresh)
                
                proposals_per_fpn_level_per_image[level_name] = moved_anchors_cropped[idxs_post_nms]
                
                scores_per_fpn_level_per_image[level_name] = level_obj_logits[idxs_post_nms]
                ##############################################################
                #                        END OF YOUR CODE                    #
                ##############################################################
