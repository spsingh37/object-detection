...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        pd = (pad, pad, pad, pad)
        x_copy = torch.nn.functional.pad(x_copy, pd)
        N = x_copy.shape[0]
        F = w.shape[0]
        H_in = x.shape[2]
        W_in = x.shape[3]
        K_H_in = w.shape[2]
        K_W_in = w.shape[3]
        H_out = (H_in + 2*pad - K_H_in)//stride + 1
        W_out = (W_in + 2*pad - K_W_in)//stride + 1
          ...
          for f in range(F):
            for i in range(H_out):
              for j in range(W_out):
                mat_t_conv = x_copy[n,:,stride*i:stride*i+K_H_in,stride*j:stride*j+K_W_in]
                conv_op = torch.sum(torch.mul(mat_t_conv,w[f,:,:,:]))
                out[n,f,i,j] = conv_op + b[f]
        #####################################################################
        #                          END OF YOUR CODE                         #
        #####################################################################
        cache = (x, w, b, conv_param)
    ...
    def backward(dout, cache):
        ...
        stride = conv_param['stride']
        pd = (pad, pad, pad, pad)
        x_pad = torch.nn.functional.pad(x, pd)
        ...
        for n in range(N):
          for f in range(F):
            ...
            for i in range(H_out):
              for j in range(W_out):
                mat_t_conv = x_pad[n, : , i*stride:i*stride+K_H_in, j*stride:j*stride+K_W_in]
                dw[f, :, :, :] += torch.mul(mat_t_conv,dout[n, f, i, j])
                
                dx[n, :, i*stride:i*stride+K_H_in,j*stride:j*stride+K_W_in] += torch.mul(w[f, :, :, :], dout[n, f, i, j])
...
class MaxPool(object):
    ...
    def forward(x, pool_param):
        ...
        H_in = x_copy.shape[2]
        W_in = x_copy.shape[3]
        K_H_in = pool_param['pool_height']
        K_W_in = pool_param['pool_width']
        stride = pool_param['stride']
        H_out = (H_in - K_H_in)//stride + 1
        W_out = (W_in - K_W_in)//stride + 1
        ...
        for n in range(N):
          for c in range(C):
            for i in range(H_out):
              for j in range(W_out):
                mat_t_pool = x_copy[n,c,stride*i:stride*i+K_H_in,stride*j:stride*j+K_W_in]
                max_elem = torch.max(mat_t_pool)
                out[n,c,i,j] = max_elem
        ####################################################################
        #                         END OF YOUR CODE                         #
        ####################################################################
        cache = (x, pool_param)
    ...
    def backward(dout, cache):
        ...
        for n in range(N):
          for c in range(C):
            for i in range(H_out):
              for j in range(W_out):
                mat_t_pool = x[n, c, i*stride:i*stride+K_H_in, j*stride:j*stride+K_W_in]
                w = torch.ones_like(mat_t_pool)
                sub_max = mat_t_pool-torch.max(mat_t_pool)
                w[sub_max<0] = 0
                dx[n, c, i*stride:i*stride+K_H_in,j*stride:j*stride+K_W_in] += torch.mul(w, dout[n, c, i, j])  
        ####################################################################
        #                          END OF YOUR CODE                        #
        ####################################################################
        return dx
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        ######################################################################
        # Replace "pass" statement with your code
        C_in,H_in,W_in = input_dims[0], input_dims[1], input_dims[2]
        C_out = num_filters
        K = filter_size
        self.params['W1'] = weight_scale*torch.randn(C_out,C_in,K,K,device=device,dtype=self.dtype)
        self.params['b1'] = torch.zeros(C_out,device=device,dtype=self.dtype)
        H_out = H_in//2
        W_out = W_in//2
        D = C_out*H_out*W_out
        self.params['W2'] = weight_scale*torch.randn(D,hidden_dim,device=device,dtype=self.dtype)
        self.params['b2'] = torch.zeros(hidden_dim,device=device,dtype=self.dtype)
        self.params['W3'] = weight_scale*torch.randn(hidden_dim,num_classes,device=device,dtype=self.dtype)
    ...
    def loss(self, X, y=None):
        ...
        lin_rl_out, lin_rl_cache = Linear_ReLU.forward(flat_out,W2,b2)
        scores, lin_cache = Linear.forward(lin_rl_out,W3,b3)
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
        ...
        ####################################################################
        # Replace "pass" statement with your code
        loss,upstr_grad = softmax_loss(scores,y)
        
        dlin_x,grads['W3'],grads['b3'] = Linear.backward(upstr_grad,lin_cache)
        dlin_rl_x,grads['W2'],grads['b2'] = Linear_ReLU.backward(dlin_x,lin_rl_cache)
        dflat_in = dlin_rl_x.reshape_as(conv_out)
        _,grads['W1'],grads['b1'] = Conv_ReLU_Pool.backward(dflat_in,conv_cache)
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        K_pool = 2
        P = 1
        self.pool = torch.zeros(len(num_filters))
        ...
        for i in range(len(num_filters)):
          ...
          if weight_scale=="kaiming":
            self.params[f"W{i+1}"] = kaiming_initializer(C_in,C_out,K_conv,relu=True,device=device,dtype=self.dtype)
          else:
            self.params[f"W{i+1}"] = weight_scale*torch.randn(C_out,C_in,K_conv,K_conv,device=device,dtype=self.dtype)
          self.params[f"b{i+1}"] = torch.zeros(C_out,device=device,dtype=self.dtype)
          if self.batchnorm:
              self.params['gamma'+str(i+1)] = torch.ones(C_out, dtype=dtype, device=device)
              self.params['beta' +str(i+1)] = torch.zeros(C_out, dtype=dtype, device=device)
          C_in = C_out
          H_conv_out = (H_in-K_conv+2*P)//S_conv + 1
          W_conv_out = (W_in-K_conv+2*P)//S_conv + 1
          if self.pool[i] == 1:
            H_out = (H_conv_out-K_pool)//S_pool + 1 
            W_out = (W_conv_out-K_pool)//S_pool + 1
          else:
            H_out = H_conv_out
            W_out = W_conv_out
          H_in = H_out
        ...
        D = C_out*H_out*W_out
        if weight_scale=="kaiming":
          self.params[f"W{i+2}"] = kaiming_initializer(D,num_classes,relu=False,device=device,dtype=self.dtype)
        else:
          self.params[f"W{i+2}"] = weight_scale*torch.randn(D, num_classes, dtype = dtype, device = device)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
          if self.pool[i]==1:
              if self.batchnorm:
                  ...
                  conv_out, conv_cache[f"{i+1}"] = Conv_BatchNorm_ReLU_Pool.forward(prev,self.params[f"W{i+1}"],self.params[f"b{i+1}"],gamma, \
                                                     beta,conv_param,bn_param,pool_param)
              else:
                  conv_out, conv_cache[f"{i+1}"] = Conv_ReLU_Pool.forward(prev, self.params[f"W{i+1}"], self.params[f"b{i+1}"], conv_param, pool_param)
          else:
              if self.batchnorm:
                  gamma = self.params['gamma'+str(i+1)]
                  beta = self.params['beta'+str(i+1)]
                  bn_param = self.bn_params[i]
                  conv_out, conv_cache[f"{i+1}"] = Conv_BatchNorm_ReLU.forward(prev,self.params[f"W{i+1}"],self.params[f"b{i+1}"],gamma, \
                                                     beta, conv_param, bn_param)
              else:
                  conv_out, conv_cache[f"{i+1}"] = Conv_ReLU.forward(prev, self.params[f"W{i+1}"], self.params[f"b{i+1}"], conv_param)
          prev = conv_out.clone()
                    
        flat_out = conv_out.reshape(conv_out.shape[0],-1)
        scores, lin_cache = Linear.forward(flat_out,self.params[f"W{i+2}"],self.params[f"b{i+2}"])
        #####################################################
        #                 END OF YOUR CODE                  #
        #####################################################
        ...
        for i in reversed(range(self.num_layers)):
          if i==self.num_layers-1:
            dlin_x, grads[f"W{i+1}"], grads[f"b{i+1}"] = Linear.backward(upstr_grad,lin_cache)
            dx = dlin_x.reshape_as(conv_out)
          elif self.pool[i]==1:
              if self.batchnorm:
                  dx, grads[f"W{i+1}"], grads[f"b{i+1}"], grads[f"gamma{i+1}"], grads[f"beta{i+1}"] = Conv_BatchNorm_ReLU_Pool.backward(dx, conv_cache[f"{i+1}"])
              else:
                  dx, grads[f"W{i+1}"], grads[f"b{i+1}"] = Conv_ReLU_Pool.backward(dx,conv_cache[f"{i+1}"])
          else:
              if self.batchnorm:
                  dx, grads[f"W{i+1}"], grads[f"b{i+1}"], grads[f"gamma{i+1}"], grads[f"beta{i+1}"] = Conv_BatchNorm_ReLU.backward(dx, conv_cache[f"{i+1}"])
              else:
                  dx, grads[f"W{i+1}"], grads[f"b{i+1}"] = Conv_ReLU.backward(dx,conv_cache[f"{i+1}"])
...
def create_convolutional_solver_instance(data_dict, dtype, device):
    ...
    model = DeepConvNet(input_dims=input_dims, num_classes=10,
                    ...
                    max_pools=[0, 1],
                    reg=10e-5, weight_scale='kaiming', dtype=dtype, device=device)
    
    solver = Solver(model, small_data,
                print_every=1000, num_epochs=14, batch_size=128,
                update_rule=adam,
                optim_config={
...
def kaiming_initializer(Din, Dout, K=None, relu=True, device='cpu',
    ...
    if K is None:
        ...
        weight_scale = torch.sqrt(gain/fan_in)
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == 'train':
            ...
            cache = {"x":x, "mode":"train"}
            sample_mean = (torch.sum(x,dim=0)/x.shape[0]).reshape(1,-1)
                        
            x_mins_x_bar = x - sample_mean
            
            x_mins_x_bar_sq = torch.multiply(x_mins_x_bar,x_mins_x_bar)
            sample_variance = (torch.sum(x_mins_x_bar_sq,dim=0)/(x.shape[0])).reshape(1,-1)
            
            x_norm = (x - sample_mean)/torch.sqrt(sample_variance + eps)
            
            out = torch.multiply(x_norm,gamma.reshape(1,-1)) + beta.reshape(1,-1)
            ...
            cache['std'] = (sample_variance + eps) ** 0.5
            ...
            # Replace "pass" statement with your code
            cache = {"x":x, "mode":"test"}
            running_mean = running_mean.reshape(1,-1)
            running_var = running_var.reshape(1,-1)
    ...
    def backward(dout, cache):
        ...
        if mode=='train':
            ...
            d_out_x_norm = gamma
            d_x_norm = torch.multiply(dout, d_out_x_norm)

            d_x_norm_x_minus_x_mean_1 = 1/std
            d_x_minus_x_mean_1 = torch.multiply(d_x_norm_x_minus_x_mean_1,d_x_norm)
            d_x_norm_std = x-mean
            d_std = torch.sum(d_x_norm * d_x_norm_std, dim=0)

            d_std_sqrt = -1/(std**2)
            d_sqrt = d_std * d_std_sqrt
          
            d_sqrt_sum_x_i = 0.5/((var+eps)**0.5)
            d_sum_x_i = d_sqrt_sum_x_i * d_sqrt
          
            d_sum_x_i_sq = 1/N
            d_sq = d_sum_x_i_sq*d_sum_x_i
          
            d_sq_x_minus_x_mean_2 = 2*(x-mean)
            d_x_minus_x_mean_2 = d_sq_x_minus_x_mean_2*d_sq
          
            d_x_minus_x_mean = d_x_minus_x_mean_1 + d_x_minus_x_mean_2
          
            d_x_minus_x_mean_x = 1
            d_x_1 = d_x_minus_x_mean*d_x_minus_x_mean_x
          
            d_x_minus_x_mean_x_mean = -1
            d_x_mean = torch.sum(d_x_minus_x_mean_x_mean*d_x_minus_x_mean,dim=0)
          
            d_x_mean_x = 1/N
            d_x_2 = d_x_mean_x*d_x_mean
            ...
            dx = torch.multiply(dout,gamma*(1-running_mean)/((running_var+eps)**0.5))
        else:
    ...
    def backward_alt(dout, cache):
        ...
        dx = gamma/torch.sqrt(var + eps) * (dout - torch.sum(dout, 0)/N - x_norm/N * torch.sum(dout * x_norm, 0))
        #################################################################
        #                        END OF YOUR CODE                       #
        #################################################################
...
class SpatialBatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        x_normed, cache = BatchNorm.forward(x_reshaped, gamma, beta, bn_param)
        out = x_normed.reshape(N, H, W, C).permute(0,3,1,2)
        ################################################################
        #                       END OF YOUR CODE                       #
        ################################################################
    ...
    def backward(dout, cache):
        ...
        dx_, dgamma, dbeta = BatchNorm.backward_alt(dout_reshaped, cache)
        dx = dx_.reshape(N, H, W, C).permute(0,3,1,2)
        ##################################################################
        #                       END OF YOUR CODE                         #
        ##################################################################
